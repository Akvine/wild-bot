...
--preconditions onFail:MARK_RAN onError:HALT onUpdateSQL:FAIL
--precondition-sql-check expectedResult:0 select count(*) from information_schema.tables where upper(table_name) = 'CLIENT_SUBSCRIPTION_ENTITY' and table_schema = 'public';
CREATE TABLE CLIENT_SUBSCRIPTION_ENTITY
(
    ID                          BIGINT       NOT NULL,
    CLIENT_ID                   BIGINT       NOT NULL,
...
CREATE SEQUENCE SEQ_CLIENT_SUBSCRIPTION_ENTITY START WITH 1 INCREMENT BY 1000;
CREATE UNIQUE INDEX CLIENT_SUBSCRIPTION_ID_INDEX ON STATE_ENTITY (ID);
CREATE UNIQUE INDEX CLIENT_SUBSCRIPTION_CLIENT_ID_INDEX ON CLIENT_SUBSCRIPTION_ENTITY (CLIENT_ID);