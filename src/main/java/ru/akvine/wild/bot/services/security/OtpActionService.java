...
public abstract class OtpActionService<T extends OneTimePasswordable> {
    ...
    public <R> R generateNewOtp(String payload) {
        ...
        T otpAction = lockProvider.doWithLock(lockId, () -> {
            ...
            if (action.getOtpAction().isActionExpired()) {
                logger.info(
                        "Client with email = {} tried to get new otp, but {} is expired",
                        action.getLogin(),
                        getActionName());
    ...
    protected T checkOtpInput(String payload, String clientInput, String sessionId) {
        ...
        if (action.getOtpAction().isExpiredOtp()) {
            logger.info(
                    "User with email = {} tried to finish {}, but otp is expired! New otp left = {}",
                    login,
                    getActionName(),
    ...
    protected void verifyNotBlocked(String login) {
        ...
        logger.info(
                "Blocked email = {} trying to perform action = {}. Block until date = {}",
                login,
                getActionName(),
                unblockDate);
        throw new BlockedCredentialsException(login);
    ...
    protected void handleNoMoreNewOtp(T action) {
        ...
        logger.info(
                "Client with email = {} reached limit of maximum otp generation for {} and set blocked!",
                login,
                getActionName());
        getRepository().delete(action);
        logger.info(
                "Blocked client's action = {} for email = {}[id = {}] removed from DB",
                login,
                getActionName(),
                action.getId());
    }
    ...
    private boolean newOtpDelayIsNotPassed(T passwordChangeAction) {
        LocalDateTime generationAllowedTime =
                passwordChangeAction.getOtpAction().getOtpLastUpdate().plusSeconds(otpDelaySeconds);
