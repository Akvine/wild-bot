...
public class FacadesConfig {
    ...
    public StateResolverFacade stateResolverFacade(List<StateResolver> stateResolvers) {
        Map<ClientState, StateResolver> stateResolversMap =
                stateResolvers.stream().collect(toMap(StateResolver::getState, identity()));
    ...
    public CommandResolverFacade commandResolverFacade(List<CommandResolver> commandResolvers) {
        Map<Command, CommandResolver> commandResolverMap =
                commandResolvers.stream().collect(toMap(CommandResolver::getCommand, identity()));
        return new CommandResolverFacade(commandResolverMap);
    }

    @Bean
    public BotViewFacade telegramEventFacade(List<BotView> botViews) {
        Map<ClientState, BotView> keyboardMap = botViews.stream().collect(toMap(BotView::byState, identity()));
        return new BotViewFacade(keyboardMap);
    }

    @Bean
    public QrCodeGenerationServiceFacade qrCodeGenerationServiceFacade(
            List<QrCodeGenerationService> qrCodeGenerationServices) {
    ...
    public SensitivePropertyMaskersFacade sensitivePropertyMaskersFacade(List<PropertyMasker> maskers) {
        Map<SensitiveDataType, PropertyMasker> map =
                maskers.stream().collect(toMap(PropertyMasker::getType, identity()));
    ...
    public WildberriesProxiesFacade wildberriesProxiesFacade(List<WildberriesIntegrationServiceProxy> proxies) {
        Map<ProxyType, WildberriesIntegrationServiceProxy> proxiesMap =
                proxies.stream().collect(toMap(WildberriesIntegrationServiceProxy::getType, identity()));
    ...
    public BotDtoConverterFacade botDtoConverterFacade(List<BotDtoConverter<?, ?>> converters) {
        Map<BotType, BotDtoConverter<?, ?>> convertersMap =
                converters.stream().collect(toMap(BotDtoConverter::getType, identity()));
