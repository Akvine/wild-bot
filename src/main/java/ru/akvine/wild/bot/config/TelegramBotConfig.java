...
public class TelegramBotConfig {
    ...
    public DefaultBotOptions defaultBotOptions() {
        ...
        if (environment.getProperty(HTTPS_PROXY_PORT_PROPERTY_NAME) == null
                || environment.getProperty(HTTPS_PROXY_HOST_PROPERTY_NAME) == null) {
            return defaultBotOptions;
        }
        String proxyHost = environment.getProperty(HTTPS_PROXY_HOST_PROPERTY_NAME);
        int proxyPort =
                Integer.parseInt(Objects.requireNonNull(environment.getProperty(HTTPS_PROXY_PORT_PROPERTY_NAME)));
    ...
    private void setWebhook(String botToken) {
        ...
        String botPathWithSecret = StringUtils.endsWith(botPath, "/") ? botPath + botSecret : botPath + "/" + botSecret;

        if (!getWebhookInfoResponse.isOk()
                || !botPathWithSecret.equals(getWebhookInfoResponse.getResult().getUrl())) {
    ...
    private GetWebhookInfoResponse sendGetWebhookInfoRequest(String botToken) {
        ...
        return restTemplate.getForObject(request.getTelegramRequestMethod(), GetWebhookInfoResponse.class);
    }
    ...
    private SetWebhookResponse sendSetWebhookRequest(String botToken, String botPath) {
        ...
        return restTemplate.postForObject(request.getTelegramRequestMethod(), null, SetWebhookResponse.class);
    }
